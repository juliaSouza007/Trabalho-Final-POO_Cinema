...
public class VendasDAO {
    ...
    public static void criaTabela() throws FalhaConexaoException {
        ...
        try {
            ...
            stmt.execute("CREATE TABLE IF NOT EXISTS `coltec`.`Vendas` (" +
                    "`id` INT NOT NULL," +
                    "`data` DATETIME NOT NULL," +
                    "`cinema_id` INT NOT NULL," +
                    "`valor_total` DOUBLE NOT NULL," +
                    "PRIMARY KEY (`id`)), " +
                    "INDEX `fk_Vendas_Cinema_idx` (`cinema_id` ASC)," +
                    "CONSTRAINT `fk_Vendas_Cinema` FOREIGN KEY (`cinema_id`)" +
                    "REFERENCES `coltec`.`Cinema` (`id`) ON DELETE CASCADE)," +
                    "ENGINE = InnoDB;");
    ...
    public static void inserir(Vendas venda) throws FalhaConexaoException {
        String sql = "INSERT INTO Vendas (id, data, cinema_id, valor_total) VALUES (?, ?, ?, ?)";
        try (Connection conexao = Conexao.obtemConexao();
             PreparedStatement stmt = conexao.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {

            stmt.setInt(1, venda.getId());
            stmt.setTimestamp(2, Timestamp.valueOf(venda.getData().toLocalDateTime()));
            stmt.setInt(3, venda.getCinema().getId());
